import { SYMM_TOKEN, uncappedTokens, equivalentSets } from './tokens';
...
export function getStakingBoostOfPair(
    ...
    if (token1 == SYMM_TOKEN && uncappedTokens.includes(token2)) {
        return balMultiplier
            .times(weight1)
            .plus(weight2)
            .div(weight1.plus(weight2));
    } else if (token2 == SYMM_TOKEN && uncappedTokens.includes(token1)) {
        return weight1
