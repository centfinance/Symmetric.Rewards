...
export function sumUserLiquidity(pools, bal_per_snapshot) {
    console.log(
        `bal_per_snapshot--->>>>>>>> ${JSON.stringify(bal_per_snapshot)}`
    );
    ...
    console.log(`bal_per_snapshot OK ${JSON.stringify(pools)}`);
    ...
    for (const pool of pools) {
        ...
        if (isPrivatePool) {
            ...
            //"\n"
            //);
            console.log(`shared Pools--->>>>>>>> ${JSON.stringify(userPools)}`);
    ...
    console.log(
        `userBalReceived--->>>>>>>> ${JSON.stringify(userBalReceived)}`
    );
    console.log(`totalUserBal--->>>>>>>> ${JSON.stringify(totalUserBal)}`);
