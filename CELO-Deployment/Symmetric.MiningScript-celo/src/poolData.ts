...
import { uncappedTokens, SYMM_TOKEN, SYMM_TOKEN2 } from './tokens';
...
export function splitLiquidityProviders(
    ...
    let includesBal: boolean = poolTokens.includes(SYMM_TOKEN) || poolTokens.includes(SYMM_TOKEN2);
    let includesUncappedTokenPair: boolean = poolTokens.reduce(
        (found, token) => {
            return (
                found ||
                (token !== SYMM_TOKEN && token !== SYMM_TOKEN2 && uncappedTokens.includes(token))
            );
        },
        false
    );
