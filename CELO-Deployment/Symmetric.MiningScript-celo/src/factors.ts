import { SYMM_TOKEN, SYMM_TOKEN2, uncappedTokens, equivalentSets } from './tokens';
...
export function getStakingBoostOfPair(
    ...
    if ((token1 == SYMM_TOKEN || token1 == SYMM_TOKEN2) && uncappedTokens.includes(token2)) {
        return balMultiplier
            .times(weight1)
            .plus(weight2)
            .div(weight1.plus(weight2));
    } else if ((token2 == SYMM_TOKEN || token2 == SYMM_TOKEN2) && uncappedTokens.includes(token1)) {
        return weight1
...
export function getBalAndRatioFactor(tokens, weights, balMultiplier = bnum(2)) {
    let brfSum = bnum(0);
